...
class Usuario:
    ...
    def asignar_presupuesto(self, categoria, monto):
        ...
        if monto < 0:
            raise Exception(
                "Monto invalido para el presupuesto (debe ser mayor o igual a 0)"
            )

        # Si ya había un presupuesto previo, calcular la diferencia (delta)
        previo = float(getattr(categoria, "presupuestoInicial", 0) or 0)
        delta = monto - previo

        # Si delta es positivo, necesitamos comprobar que el usuario tenga saldo suficiente
        if delta > 0 and delta > self.saldoRestante:
            raise Exception(
                "Monto invalido para el presupuesto (supera saldo disponible del usuario)"
            )

        # Asignar nuevo presupuesto y ajustar saldoRestante de la categoría teniendo en cuenta gastos ya realizados
        categoria.presupuestoInicial = monto
        # el saldo restante de la categoría es el presupuesto menos lo ya gastado (no menos del 0)
        try:
            categoria.saldoRestante = max(
                monto - float(getattr(categoria, "gastos", 0) or 0), 0
            )
        except Exception:
            categoria.saldoRestante = monto

        # Ajustar el saldo del usuario según la diferencia (delta). Si delta negativo, esto devuelve dinero al usuario.
        self.saldoRestante -= delta
    ...
    def registrar_gasto(self, categoria, monto, descripcion=""):
        ...
        fecha = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # obtenemos la fecha y hora actual, strftime la formatea como cadena esto tambien se hizo con ayuda de IA
        movimiento = {
            "id": str(
                uuid.uuid4()
            ),  # generamos un ID unico para el movimiento con uuid4
    ...
    def Alertas(self):
        ...
        if self.ingreso_mensual > 0:
            ...
            if porcentaje_gasto >= 80:
                alertas.append(
                    f"Alerta: Ha gastado el {porcentaje_gasto:.2f}% de su ingreso mensual."
                )
        ...
        for nombre, categoria in [
            ...
            ("Hogar", self.hogar),
            ("Otros", self.otros),
        ]:  # lista de categorias y sus objetos correspondientes
            if categoria.presupuestoInicial > 0:
                porcentaje_categoria = (
                    categoria.gastos / categoria.presupuestoInicial
                ) * 100
                if porcentaje_categoria >= 80:
                    alertas.append(
                        f"Alerta: Ha gastado el {porcentaje_categoria:.2f}% del presupuesto asignado para la categoría {nombre}."
                    )
        return alertas
    ...
    def eliminar_gasto_por_id(self, gasto_id):
        ...
        for movimiento in list(self.historial):
            ...
            categoria_nombre = (
                movimiento.get("categoria") or movimiento.get("categoria_nombre") or ""
            )
            categoria_obj = None
            for cat in [self.alimentacion, self.transporte, self.hogar, self.otros]:
                try:
                    if getattr(cat, "nombre", "").lower() == str(
                        categoria_nombre
                    ).lower() or cat is getattr(self, categoria_nombre.lower(), None):
                        categoria_obj = cat
                        break
                except Exception:
                    continue
            ...
            try:
                ...
                if categoria_obj and movimiento in getattr(
                    categoria_obj, "movimientos", []
                ):
